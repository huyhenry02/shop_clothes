...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="POST" action="{{ route('admin.category.putCategory', $category->id) }}" enctype="multipart/form-data">
                @csrf
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let existingSizes = "{{ $category->sizes ?? '' }}".split(",");
            existingSizes.forEach(size => {
                if (size.trim() !== "") sizes.push(size.trim());
            });
            updateSizeDisplay();
